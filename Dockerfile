...
# Start both gateway (with allow-unconfigured) and wrapper
CMD ["sh", "-c", "openclaw gateway run --allow-unconfigured --auth token --token iao-fund-gateway-token-2026 & sleep 10 && node /wrapper.js"]
